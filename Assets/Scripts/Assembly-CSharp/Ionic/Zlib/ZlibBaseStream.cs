...
namespace Ionic.Zlib
{
	internal class ZlibBaseStream : Stream
	{
		...
		private void finish()
		{
			...
			if (this._streamMode == ZlibBaseStream.StreamMode.Writer)
			{
				...
				if (this._flavor == ZlibStreamFlavor.GZIP)
				{
					...
					int value = (int)(this.crc.TotalBytesRead & uint.MaxValue);
			...
			else if (this._streamMode == ZlibBaseStream.StreamMode.Reader && this._flavor == ZlibStreamFlavor.GZIP)
			{
				...
				int num6 = (int)(this._z.TotalBytesOut & uint.MaxValue);
		...
		protected internal StreamMode _streamMode = ZlibBaseStream.StreamMode.Undefined;
