...
public class ScrollList : Widget, WidgetListener
{
	...
	public void AddButton(Widget button)
	{
		...
		this.PlaceButtons(ScrollList.Action.Place);
	}
	...
	private float ScreenWidth()
	{
		...
		{
			return 20f * (float)Screen.width / (float)Screen.height - this.leftMargin;
		}
	...
	private void Start()
	{
		...
		if (this.fillScreenWidth)
		{
			...
			if (num > 10f * (float)Screen.width / (float)Screen.height - this.leftMargin)
			{
	...
	private void OnDrawGizmos()
	{
		...
		{
			this.PlaceButtons(ScrollList.Action.DrawGizmos);
		}
	...
	{
		if (action == ScrollList.Action.Place)
		{
