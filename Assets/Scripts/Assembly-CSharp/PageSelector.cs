...
public class PageSelector : MonoBehaviour
{
	...
	public int CurrentPage
	{
		get
		{
			return Mathf.Clamp(Mathf.RoundToInt(this.m_scrollPivot.transform.localPosition.x / -this.m_hudCamera.ScreenToWorldPoint(new Vector3((float)Screen.width, 0f, 0f)).x), 0, this.m_pageCount - 1);
		}
	}
	...
	public List<GameObject> Elements
	{
		get
		{
			return this.m_pageElements;
		}
	}

	private void Awake()
	{
		if (this.m_instantiateMenuBackground)
		{
	...
	private void Update()
	{
		...
		if (!this.m_interacting)
		{
			...
			if (!DeviceInfo.UsesTouchInput)
			{
				...
				if (this.CurrentPage == this.m_pageCount - 1 || this.m_pageCount == 1)
				{
					this.m_rightScroll.SetActive(false);
				}
			}
	...
	private void Layout()
	{
		...
		int count = this.m_pageElements.Count;
		float num2 = this.EdgeMargin + this.m_elementWidth / 2f + this.m_scrollButtonMargin;
		float y = 0f;
		Vector3 vector = new Vector3(-num / 2f + num2, y);
		float num3 = num - 2f * num2;
		float num4 = num3 / (float)(count - 1);
		if (num4 < this.MinGap || this.m_maxElementsPerPage > 0)
		{
			this.EdgeMargin = 2.5f;
			num2 = this.EdgeMargin + this.m_elementWidth / 2f + this.m_scrollButtonMargin;
			y = 0f;
			vector = new Vector3(-num / 2f + num2, y);
			num3 = num - 2f * num2;
			this.m_elementsPerPage = (int)(num3 / this.MinGap) + 1;
			if (this.m_maxElementsPerPage > 0 && this.m_elementsPerPage > this.m_maxElementsPerPage)
			{
				this.m_elementsPerPage = this.m_maxElementsPerPage;
			}
			vector.x = -((float)(this.m_elementsPerPage - 1) * this.MinGap) / 2f;
			num4 = this.MinGap;
			this.m_pageCount = this.m_pageElements.Count / this.m_elementsPerPage + ((this.m_pageElements.Count % this.m_elementsPerPage != 0) ? 1 : 0);
		}
		...
		if (this.m_pageCount == 1)
		{
			...
			{
				this.m_pageElements[i].transform.position = vector + Vector3.right * num4 * (float)i;
			}
		}
		else
		{
			...
			while (j < count)
			{
				int num6 = this.m_elementsPerPage;
				if (num6 > count - j)
				{
					num6 = count - j;
				}
				...
				{
					this.m_pageElements[j].transform.position = vector + Vector3.right * num4 * (float)k;
					j++;
					if (j >= count)
					{
						break;
	...
	public int GetElementPage(GameObject element)
	{
		...
		float num2 = this.EdgeMargin + this.m_elementWidth / 2f + this.m_scrollButtonMargin;
		float num3 = num - 2f * num2;
		int num4 = (int)(num3 / this.MinGap) + 1;
		if (this.m_maxElementsPerPage > 0 && num4 > this.m_maxElementsPerPage)
		{
			num4 = this.m_maxElementsPerPage;
		}
		for (int i = 0; i < this.m_pageElements.Count; i++)
		{
			if (this.m_pageElements[i] == element)
			{
				return i / num4;
			}
		}
	...
	{
		if (DeviceInfo.ActiveDeviceFamily != DeviceInfo.DeviceFamily.Android && obj == KeyCode.RightArrow && this.m_rightScroll.activeInHierarchy)
		{
			this.NextPage();
		}
